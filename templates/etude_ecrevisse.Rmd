...
subtitle: "Données ASPE, ASTAQUITAINE, FAUNA, FDAAPPMA 64, NAIADES et OISON"
...
La base de données de travail est une jointure de plusieurs bases distinctes :
...
- **ASTAQUITAINE** : base de données réalisée dans le cadre du Programme Aquitain Ecrevisses à Pattes blanches. Elle centralise les données issues d'inventaires piscicoles par pêche électrique et d'inventaires astacicoles par prospection nocturne ou piégeage. *Date de l'extraction : 10/02/2026*
...
- **FDAAPPMA 64** : données issues pour partie de la base de données Géopikaia. *Date de l'extraction : 07/02/2026*

Un traitement des données avant et après jointure a été réalisé en supprimant des données OFB et autres doublons. Dans la base FAUNA, une partie des données OFB étaient déjà présentes (remontée des données via le SINP), il faut donc supprimer ces doublons pour limiter les biais. Les doublons ont été identifiés par croisement selon plusieurs variables : la date journalière, le code identifiant de l'espèce et les coordonnées gps. Ce même processus est appliqué entre les bases ASPE, NAIADES et ASTAQUITAINE. Cela a conduit à la suppression de **894 doublons**.
La base finale est donc constituée de **`r nrow(bdd_ecrevisse)`** données d'observations d'écrevisses sur l'ensemble de la région Nouvelle-Aquitaine.

```{r, fig.width = 6, fig.height = 4}
tablo_data <- data.frame(
  Base = c("ASPE", "ASTAQUITAINE","FAUNA","FDAAPPMA 64", "NAIADES","OISON"),
  Donnees_brutes = c(nrow(data_aspe), nrow(data_astaq), nrow(data_fauna), nrow(data_64), nrow(data_naiades),nrow(data_oison)),
...
flextable(tablo_data) %>%
  set_header_labels(
    ...
    Donnees_brutes = "Données traitées") %>%
  theme_vanilla() %>%
...
ggplot(histo_synt_esp) +
  ...
  scale_color_manual(values = colors_esp,
                     name = "Espèces") +
  scale_y_continuous(breaks = seq(0, max(histo_synt_esp$nb_saisies), by = 200)) +
...
(carte_esp1 | carte_esp5 | carte_esp6) / (carte_esp2 | carte_esp3 | carte_esp4 | carte_esp7)
```
...
Ici, ce sont bien les **données d'effectifs** qui sont analysées, de nombreuses observations ne contiennent pas de données d'effectifs.
...
flextable(carac_esp_spe) %>%
  set_header_labels(
    Nom_vernaculaire = "Espèce",
    nb_obs_2005 = "Individus \navant 2005",
    nb_obs_2005_2015 = "Individus \nentre 2005 et 2015",
    nb_obs_2015_2020 = "Individus \nentre 2015 et 2020",
    nb_obs_2025 = "Individus \naprès 2020") %>%
  theme_vanilla() %>%
  color(part = "header", color = "white") %>%
  bg(part = "header", bg = "#333333") %>%
  bold(part = "header") %>% 
...
Cartographie par espèce, du **nombre d'individus** observés selon les trois mêmes périodes temporelles. Du fait du très peu nombre de données d'effectifs pour l'écrevisse calicot (1 individu) et l'écrevisse à pattes rouges (304 individus avant 2005, aucune information d'effectif depuis) ne sont pas représentées dans les cartes suivantes. 
...
cartes_162666 <- (carte_162666_2005 | carte_162666_2015 | carte_162666_2020) +
  plot_annotation(
    theme = theme(
      plot.title = element_text(size = 14),
      plot.subtitle = element_text(size = 8)
...
```{r, fig.width = 18, fig.height = 18}
cartes_162667 / cartes_162668 / cartes_17646
```
...
tablo_abs <- bdd_ecrevisse %>%
  st_set_geometry(NULL) %>%
  mutate(Presence = dplyr::recode(Presence,
                           "oui" = "Présent",
                           "non" = "Absent")) %>%
  group_by(Presence) %>%
